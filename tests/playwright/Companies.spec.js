...
test('Companies', async ({ page }) => {
    ...
    var company=await companies_add_from_Companies(page)

    // Update Companies
    await page.getByTestId(`Companies_Table_IconEdit${company.id}`).click()
    ...
    // Delete Companies
    await page.getByTestId(`Companies_Table_IconDelete${company.id}`).click()
