...
export async function biometrics_add_from_Biometrics(page){
    ...
    const response_promise = promise_to_get_response(page, "/api/biometrics/", "POST");
    await page.getByTestId('BiometricsCRUD_Button').click()
    const response = await response_promise;
    await expect(page.getByTestId(`Biometrics_Table_IconEdit${response.id}`)).toBeVisible();
    return response
}
export async function companies_add_from_Companies(page, name="My Linux company"){
    ...
    const response_promise = promise_to_get_response(page, "/api/companies/", "POST");
    await page.getByTestId('CompaniesCRUD_Button').click()
    const response = await response_promise;
    await expect(page.getByTestId(`Companies_Table_IconEdit${response.id}`)).toBeVisible();
    return response
}
...
export async function meals_add_from_Meals(page){
    ...
    const response_promise = promise_to_get_response(page, "/api/meals/", "POST");
    await page.getByTestId('MealsCRUD_Button').click()
    const response = await response_promise;
    // await expect(page.getByTestId(`Meals_Table_IconEdit${response.id}`)).toBeVisible(); OUT OF SCREEN
    return response
}
...
export async function products_add_from_Products(page, name="My product"){
    ...
    const response_promise = promise_to_get_response(page, "/api/products/", "POST");
    await page.getByTestId('ProductsCRUD_ButtonAdd').click()
    const response = await response_promise;
    await expect(page.getByTestId(`Products_Table_IconEdit${response.id}`)).toBeVisible();
    return response
}
...
export async function elaboratedproducts_add_from_Products(page, name="My elaborated product"){
    ...
    const response_promise = promise_to_get_response(page, "/api/elaborated_products/", "POST");
