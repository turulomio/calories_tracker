        ...
        <v-card class="pa-5">
            <v-form  method="post">
                    ...
                    <v-card-actions class="justify-center">
                        ...
                        <v-btn color="primary" @click="submmit_auto()">{{ $t("Update from Django Calories Tracker GitHub repository")}}</v-btn>
    ...
    export default {
        ...
        methods: {
            submmit(){
                ...
                axios.post(`${this.$store.state.apiroot}/maintenance/catalogs/update/`, data, this.myheaders_formdata())
                .then((response) => {
                        if (response.data==true){
                            this.message=this.$t("Catalogs have been updated")
                            this.filename=[]
            ...
            submmit_auto(){
                if (window.navigator.onLine==false){
                    alert(this.$t("You need a Internet connection to update from Django Calories Tracker GitHub repository in Github"))
                    return
                }
                this.loading=true
                axios.post(`${this.$store.state.apiroot}/maintenance/catalogs/update/`, {auto:true,}, this.myheaders())
                .then((response) => {
                        if (response.data==true){
                            this.message=this.$t("Catalogs have been updated")
                            this.filename=[]
