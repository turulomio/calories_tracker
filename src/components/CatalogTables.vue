<template>
    <div class="ma-4">
        ...
        <v-data-table density="compact" :headers="catalog_table_headers" :items="catalog_table" :sort-by="[{key:'',order:'asc'}]"  class="elevation-1" :items-per-page="10000" :loading="loading" :key="'T'+key" height="70vh">
            ...
            <template #item.can_temperatures="{item}"><v-icon small v-if="item.raw.can_temperatures" >mdi-check-outline</v-icon><v-icon small color="red" v-if="item.man_temperatures" >mdi-check-outline</v-icon></template>
            <template #item.can_stir="{item}"><v-icon small v-if="item.raw.can_stir" >mdi-check-outline</v-icon><v-icon small color="red" v-if="item.man_stir" >mdi-check-outline</v-icon></template>   
